...
2. 找到 **「🚀 一键上线」**（AI 风格区下方）：
   - **自己用**：卡密留空 → 走**免费自用通道** → 点一键上线
   - **给别人建站（收费）**：对方提供一张**卡密** → 填进去 → 点一键上线（一人一码、用后即废）
3. 约 5~10 秒 → 页面出现网址 → **点它在浏览器打开**（带样式，不下载）→ 发给玩家即可 ✅

**说明：**
- 网站托管：腾讯云 COS 静态站点（公开只读），托管在同一 COS 桶内，数据量极小、免费额度内
- 卡密由服主线下售卖 0.2 元/张（或其他定价），用后作废
- 想换内置三套风格/让 AI 看图定制？先用「生成完整宣传站」生成并本地查看——一键上线当前使用内置默认风格（明亮简洁）
